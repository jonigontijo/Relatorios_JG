...
import { Eye, EyeOff, Pencil, Trash2, Search, Loader2 } from "lucide-react";
...
export function ClientsTable({ initialClients }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((c) => {
      const hay = [c.company, c.name, c.meta_ads_account_id, c.id]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
    ...
    <div>
      <div className="flex items-center gap-2 border-b p-3">
        <div className="relative flex-1">
          ...
          <Input
            placeholder="Buscar por nome, empresa ou ID Meta Ads"
            value={query}
